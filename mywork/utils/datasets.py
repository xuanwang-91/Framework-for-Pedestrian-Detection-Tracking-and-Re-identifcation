...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        ...
        if '*' in p:  # 如果采用正则化表达式提取图片/视频，直接使用glob获取文件路径
            files = sorted(glob.glob(p, recursive=True))  # glob
        elif os.path.isdir(p):  # 如果path是一个文件夹，使用glob获取全部文件路径
            files = sorted(glob.glob(os.path.join(p, '*.*')))  # dir
        elif os.path.isfile(p):  # 是文件则直接获取
            files = [p]  # files
        else:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            self.frame += 1
            # 打印信息
            print(f'\033[1;32m video {self.count + 1}/{self.nf}\033[0m \033[1;36m({self.frame}/{self.nframes})', end='')
            print(f'\033[1;35m {path}\033[0m ', end='')
